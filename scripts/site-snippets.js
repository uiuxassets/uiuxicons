...
export const focusTrapRuntime = `(function(){function q(p){return[].slice.call(p.querySelectorAll("a[href],button:not([disabled]),textarea:not([disabled]),input:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex=\\"-1\\"])")).filter(function(el){return el.offsetWidth>0||el.offsetHeight>0||el.getClientRects().length>0;});}window.uiuxStartFocusTrap=function(panel){var prev=document.activeElement,els=q(panel);if(els.length)els[0].focus();function onKey(e){if(e.key!=="Tab")return;var list=q(panel);if(list.length<2)return;var a=list[0],b=list[list.length-1];if(e.shiftKey&&document.activeElement===a){e.preventDefault();b.focus();}else if(!e.shiftKey&&document.activeElement===b){e.preventDefault();a.focus();}}panel.addEventListener("keydown",onKey);return function(){panel.removeEventListener("keydown",onKey);if(prev&&typeof prev.focus==="function")prev.focus();}};})();`;
